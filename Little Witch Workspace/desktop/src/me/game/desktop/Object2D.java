...
public class Object2D {
	...
	{
		float initX = rectPos.x - rectSize.x;
		float initY = rectPos.y - rectSize.y;
		float finalX = rectPos.x + rectSize.x;
		float finalY = rectPos.y + rectSize.y;

		if ((initX <= point.x && point.x <= finalX) && (initY <= point.y && point.y <= finalY)) return true;
		else return false;
	}
}
